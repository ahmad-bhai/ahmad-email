...
module.exports = async (req, res) => {
  ...
  if (token && req.method === 'POST') {
    ...
    bot.command('support', (ctx) => {
      ctx.replyWithMarkdown(`*ZQT OFFICIAL SUPPORT TEAM*`, 
        Markup.inlineKeyboard([
          Markup.button.url("📞 Contact Support", "https://t.me/zain_Quotex_Trader")
        ])
      );
    });
    ...
    bot.on('text', async (ctx) => {
      ...
      try {
        const response = await axios({
          method: 'post',
          url: 'https://ahmad-qx-bot.vercel.app/api/postback',
